...
D=M    // D = RAM[0]
@8     // A = 8
D;JGT  // условный переход: если (R0 > 0) goto -> 8
@R1    // A = 1
M=0    // иначе RAM[1]=0
@10    // A = 10
0;JMP  // безусловный переход: goto -> 10
@R1    // A = 1
M=1    // RAM[1]=1
@10    // A = 10
0;JMP  // безусловный переход: goto -> 10
...
Напишите две программы: [Mult.asm](mult/mult.asm) и [Fill.asm](fill/Fill.asm) в соответствующих папках mult и fill.

Чтобы запустить программу, откройте ее в симуляторе процессора. Симулятор процессора запускается через меню команд — ctrl+shift+p или cmd+shift+p для macOS: `Nand2Tetris: Open CPU Simulator`.
