...
2. PC `in` инпут становится равен значению в регистре А. То есть, значение в регистре А является указателем на инструкцию при осуществлении перехода.
...
Записываете в регистр экрана `1111111111111111` и закрашиваете 16-ть пикселей в черный. Чтобы закрасить весь экран в черный, нужно записать во все I/O регистры экрана 16-ть единиц.
...
@R1 // A=1
D=M // D=RAM[1]
@temp // A=temp (temp — это переменная)
M=D // RAM[temp] = R1
@R0 // A=0
D=M // D=RAM[0]
@R1 // A=1
...
Напишите две программы: [Mult.asm](mult/mult.asm) и [Fill.asm](fill/Fill.asm). Программы находятся в соответствующих папках mult и fill.
...
Чтобы запустить программу, откройте ее в симуляторе процессора.

Симулятор процессора запускается через меню команд — ctrl+shift+p или cmd+shift+p для macOS: `Nand2Tetris: Open CPU Simulator`.

Чтобы проверить программу (`fill.asm` или `mult.asm`), загрузите в симулятор процессора соответствующий тестовый файл (`fill.tst` или `mult.tst`). Тестовые файлы загружаются через кнопку с иконкой свитка. После загрузки тестового файла запускайте программу. Чтобы запустить программу нажмите на кнопку с иконкой двух стрелок (не забудьте переключить опцию `Animate` в значение `no animation`).

Если у программы нет тестового файла, например вы хотите посмотреть, как работает программа `Rect.asm` в папке `/fill`, тогда загружайте в симулятор сам `Rect.asm` файл и запускайте программу без тестового файла. Файлы `.asm` загружаются через кнопку с иконкой папки.

![как запустить программу](img/how2runProgramm.png)

### Как пользоваться клавиатурой

Чтобы использовать клавиатуру нажмите на кнопку с изображением клавиатуры, ввод с клавиатуры включится и вы сможете нажимать на клавиши.
