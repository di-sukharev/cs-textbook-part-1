...
5. Сохраните сгенерированный `.hack` файл в папку с соответствующим `.tst` файлом.
...
3. Тестовые файлы загружаются через кнопку с иконкой свитка. После загрузки тестового файла, `.hack` файл загрузится автоматически.
...
Если у программы нет тестового файла, например вы хотите посмотреть, как работает программа `Rect.asm` в папке `/fill`, тогда переводите ее в `.hack`, загружайте в симулятор `Rect.hack` файл и запускайте программу без тестового файла. Файлы `.hack` загружаются через кнопку с иконкой папки.
