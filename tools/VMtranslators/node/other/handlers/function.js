...
function pushSegment(segment) {
    return (
        pseudocode("D=POSITION", { position: segment }) + pseudocode("push(D)")
    );
}
...
function handleReturn(token) {
    ...
    commands +=
        pseudocode("D = pop()") +
        pseudocode("*POSITION=D", { position: "ARG" }); // *ARG = pop()
    commands +=
        pseudocode("D=POSITION", { position: "ARG" }) + `@SP\n` + `M=D+1\n`; // SP = ARG + 1
...
function restoreCaller(label, pos) {
    return (
        pseudocode("D = POSITION", { position: FRAME }) + // D = FRAME
        `@${pos}\n` +
        `D=D-A\n` + // D = FRAME - pos
        pseudocode("D = *D") + // D = *(FRAME - pos)
        pseudocode("POSITION = D", { position: label })
    ); // label = D = *(FRAME - pos)
}
