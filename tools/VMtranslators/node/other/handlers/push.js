...
function handlePushWithCommonSegments(token) {
    let segment =
        token.segment === "temp" ? "R5" : segmentSymbolMap[token.segment];
    return (
        pseudocode(`D = SEGMENT + i`, { segment, index: token.index }) +
...
function handlePushWithFixedSegment(token, fileName) {
    let position =
        token.segment === "static"
            ? `${fileName}.${token.index}`
            : pointerMap[token.index];
...
function handlePushWithConstantSegment(tokenOrIndex) {
    let index =
        typeof tokenOrIndex === "object" ? tokenOrIndex.index : tokenOrIndex;
    return `@${index}\n` + `D=A\n` + pseudocode("push(D)");
}
